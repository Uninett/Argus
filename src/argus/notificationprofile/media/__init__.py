...
def sync_media():
    """Sync MEDIA_CLASSES_DICT with the Media table
    ...
    for medium in Media.objects.all():
        ...
        if is_installed != medium.installed:
            ...
            if not is_installed:
                LOG.warning("%s plugin is no longer registered in MEDIA_CLASSES_DICT", medium.name)
            else:
                LOG.info("%s plugin is again registered in MEDIA_CLASSES_DICT", medium.name)
