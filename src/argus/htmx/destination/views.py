...
def _attach_delete_state(form, make_modal):
    ...
    try:
        ...
        form.delete_tooltip = " ".join(e.args)
    else:
...
class DestinationMixin:
    ...
    def _make_delete_modal(self, obj, **kwargs):
        return DeleteModal(
            ...
            explanation=f'Delete the destination "{obj}"?',
...
class DestinationUpdateView(DestinationMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        return self._render_table(
            success_message=f'Saved destination "{form.instance}" at {timezone.localtime().strftime("%H:%M:%S")}.',
        )
