...
def filter_preview(request):
    ...
    if not filter_ids:
        return render(request, "htmx/plannedmaintenance/_filter_preview.html", {"no_filters": True})

    open_incidents = Incident.objects.open()
    total_open = open_incidents.count()

    # Union of all filter results (OR logic)
    filters = Filter.objects.filter(pk__in=filter_ids)
    matching_qs = Incident.objects.none()
    for filter_obj in filters:
        matching_qs |= QuerySetFilter.filtered_incidents(filter_obj.filter, open_incidents)
    matching_qs = matching_qs.distinct()
    ...
    return render(
        request,
        "htmx/plannedmaintenance/_filter_preview.html",
        {
