...
def create_default_timeslotgroup_for_users(apps, schema_editor):
    ...
    for user in User.objects.all():
        if not user.time_slot_groups.filter(name__iexact=time_slot_group_name).exists():
            ...
            time_slot_start, time_slot_end = notification_models.TimeInterval.DAY_START, notification_models.TimeInterval.DAY_END
            for day, _day_name in notification_models.TimeInterval.DAY_CHOICES:
