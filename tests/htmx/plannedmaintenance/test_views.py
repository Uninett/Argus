...
class FilterPreviewViewTests(TestCase):
    ...
    def test_filter_preview_unions_multiple_filters(self):
        self.client.force_login(self.user)
        # Create two incidents with different sources
        incident1 = StatefulIncidentFactory()
        incident2 = StatefulIncidentFactory()

        # Create two filters, each matching one incident
        ...
        filter2 = FilterFactory(
            user=self.user,
            filter={"sourceSystemIds": [incident2.source.pk]},
        )
        ...
        # Should match both incidents (OR logic)
    ...
    def test_filter_preview_limits_incident_list(self):
        ...
        self.assertEqual(response.context["matching_count"], 7)
